...
#gecko_brunello<-scanBam('/Users/gerbix/Documents/vikas/gecko_green_monkey/gecko_brunello_20m.bam')
gecko_brunello<-scanBam('/Users/gerbix/Documents/vikas/gecko_green_monkey/brunello/brunello.bam')
...
without_pam_seq<-c()
without_pam_name<-c()
for (i in 1:length(gecko_brunello[[1]]$rname)){ 
  progress(i,65596)
  ...
  revcomp<-as.character(as.character(substr(dna[temp,], gecko_brunello[[1]]$pos[i] -3 , (gecko_brunello[[1]]$pos[i] + 20))))
  # print(temp_dna_seq)
  #  print((revcomp))
  # 
  # temp_a<-paste0(gecko_brunello[[1]]$seq[i], 'AGG')
  # temp_c<-paste0(gecko_brunello[[1]]$seq[i], 'CGG')
  # temp_t<-paste0(gecko_brunello[[1]]$seq[i], 'TGG')
  # temp_g<-paste0(gecko_brunello[[1]]$seq[i], 'GGG')
  # 
  temp_nuc<-paste0(gecko_brunello[[1]]$seq[i], '.GG')
  # temp_a_rev<-paste0('CCT',gecko_brunello[[1]]$seq[i])
  # temp_c_rev<-paste0('CCG',gecko_brunello[[1]]$seq[i])
  # temp_t_rev<-paste0('CCC',gecko_brunello[[1]]$seq[i])
  # temp_g_rev<-paste0('CCA',gecko_brunello[[1]]$seq[i])
  temp_nuc_rev<-paste0('CC.',gecko_brunello[[1]]$seq[i])
  ...
  if(grepl(temp_nuc,temp_dna_seq)){
    #print('ok')
    with_pam_names<-append(with_pam_names, gecko_brunello[[1]]$qname[i])
    ...
    # print(revcomp)
    # print('')
    without_pam_name<-append(without_pam_name, gecko_brunello[[1]]$qname[i])
