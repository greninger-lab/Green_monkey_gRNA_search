...
for (i in 1:length(gm_brunello[[1]]$rname)){ 
  ...
  if(grepl(temp_nuc,temp_dna_seq)){
    #print('ok')
    with_pam_names_pam<-append(with_pam_names_pam, gm_brunello[[1]]$qname[i])
    with_pam_seq_pam<-append(with_pam_seq_pam, temp_nuc)
  }
  else if(grepl(temp_nuc_rev,revcomp)){ 
    #print('rc')
    with_pam_names_pam<-append(with_pam_names_pam, gm_brunello[[1]]$qname[i])
    with_pam_seq_pam<-append(with_pam_seq_pam, temp_nuc_rev)
  }
  else{ 
    # print(temp_dna_seq)
    # print(revcomp)
